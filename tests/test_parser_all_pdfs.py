...
def test_single_pdf(pdf_path, degree_ids):
    ...
    try:
        ...
        course_credits_sum = sum(
            c.credits for c in data.courses
            if c.credits is not None and c.grade != "Exemption without points"
        )
        ...
        # Store extra data for detailed printing
        result["_data"] = data
...
def print_result(r):
    ...
    if r["empty_names"] > 0 and data:
        for c in data.courses:
            if not c.name.strip():
                print(f"    WARNING: Empty name for {c.course_id} (raw: {c.raw_pdf_id}) "
...
def main():
    degree, degree_ids = load_degree_ids(DEGREE_PATH)
    ...
    print(f"SUMMARY")
    print(f"{'=' * 100}")
    header = (f"{'File':<40} {'OK':>3} {'Courses':>8} {'Unique':>7} {'Dups':>5} "
              f"{'Explicit':>9} {'Wildcard':>9} {'Unmatched':>10} {'Empty':>6} {'Warnings':>9}")
    print(header)
    print("-" * len(header))
    for r in all_results:
        ok = "YES" if r["success"] else "NO"
        print(f"{r['file']:<40} {ok:>3} {r['total_courses']:>8} {r['unique_courses']:>7} "
    ...
    if total_empty_names > 0:
        ...
        print(f"         (multi-line name parsing may need improvement)")
